...
#SBATCH --job-name=sparse_pruned_dense_rs
#SBATCH --output=logs/sparse_pruned_dense_rs_%j.out
#SBATCH --error=logs/sparse_pruned_dense_rs_%j.err

set -euo pipefail

# ===========================================================================
# Ablation: grad pruning ON (sparse data), sparse reduce-scatter OFF
# Purpose: isolate the effect of sparse RS from data sparsity alone.
#   - Shim is loaded so we run ncclx NCCL (same binary as sparse RS run)
#   - USE_SPARSE_RS=0 so the shim passes through dense reduce-scatter
#   - SPARSITY=0.99 so gradients are pruned before the (dense) RS
# ===========================================================================
MODEL=5B         # 5B 10B 20B 40B 60B 80B 160B 320B 640B
MODE=fsdp        # fsdp | fsdp_tp
SPARSITY=0.99    # grad pruning on
SAMPLE_PCT=1.0   # fast approximate threshold
GBS=2048
SEQ_LEN=512
TRAIN_ITERS=20
SEED=42
...
# --- Sparse comm flags (pruning ON, sparse RS/AR OFF — ablation) ---
export USE_SPARSE_RS=0        # shim intercepts but calls dense ncclReduceScatter
...
# --- No CCD sparse collective flags (dense RS path, no channel pinning) ---
# NCCL_BUFFSIZE, NCCL_CCD_*, NCCL_MIN/MAX_NCHANNELS intentionally omitted
...
echo "Sparsity:    $SPARSITY  [PRUNING ON, sparse RS OFF — ablation]"
...
TB_DIR="$SCRIPT_DIR/tensorboard/${SLURM_JOB_ID}_${MODEL}_${MODE}_sp${SPARSITY}_dense_rs"
mkdir -p "$TB_DIR"
