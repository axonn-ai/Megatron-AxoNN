...
# Force TorchComms to use torchrun's RANK/WORLD_SIZE instead of srun's PMI_RANK/PMI_SIZE
export TORCHCOMM_BOOTSTRAP_RANKSIZE_QUERY_METHOD=torchrun

export TORCHCOMM_STORE_PATH=$PSCRATCH/sparsecomms/torchcomm_store_${SLURM_JOB_ID}
...
srun -C gpu -N $NNODES --ntasks-per-node=1 --gpus-per-node=4 \
  torchrun \
    --nnodes=$NNODES \
    --nproc_per_node=4 \
    --rdzv-backend=c10d \
