...
#SBATCH --job-name=dense_shim_ch64
#SBATCH --output=logs/dense_shim_ch64_%j.out
#SBATCH --error=logs/dense_shim_ch64_%j.err
...
SPARSITY=0.0      # 0.0 = baseline (dense), 0.99 = 99% pruning
...
# --- Sparse comm flags (dense mode — all sparse ops disabled) ---
...
TB_DIR="$SCRIPT_DIR/tensorboard/${SLURM_JOB_ID}_${MODEL}_${MODE}_dense_ncclx_ch64"
mkdir -p "$TB_DIR"
